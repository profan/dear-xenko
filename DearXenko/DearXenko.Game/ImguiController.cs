...
namespace DearXenko {
    public class ImguiController : GameSystemBase {
        ...
        public ImguiController([NotNull] IServiceRegistry registry, [NotNull] GraphicsDeviceManager graphicsDeviceManager) : base(registry) {

            input = Services.GetService<InputManager>();
            Debug.Assert(input != null, "ImGuiController: InputManager must be available!");

            deviceManager = graphicsDeviceManager;
            Debug.Assert(deviceManager != null, "ImGuiController: GraphicsDeviceManager must be available!");

            device = deviceManager.GraphicsDevice;
            Debug.Assert(device != null, "ImGuiController: GraphicsDevice must be available!");

            context = Services.GetService<GraphicsContext>();
            Debug.Assert(context != null, "ImGuiController: GraphicsContext must be available!");

            effectSystem = Services.GetService<EffectSystem>();
            Debug.Assert(effectSystem != null, "ImGuiController: EffectSystem must be available!");
        ...
        public override void Initialize() {
            ...
            Debug.Assert(c != IntPtr.Zero, "ImguiController: Failed Creating ImGui Context!");
